...
// router takes you througth diferents pages in the API
...
router.get("/game-details/:id", isLoggedIn, (req, res, next) => {
  ...
  let gameData;
  axios
    .get(`https://api.rawg.io/api/games/${id}?key=${process.env.GAMES_API_KEY}`)
    .then((gameDataParam) => {
...
router.get("/edit/:id/:name", (req, res, next) => {
  ...
  GameModel.findOne({ apiId: id }).then((response) => {
...
// router shows you the search's result in all pages in the API
router.post("/game-search", (req, res, next) => {
  axios
    .get(
      `https://api.rawg.io/api/games?key=${process.env.GAMES_API_KEY}&search=${req.body.search}`
    )
    .then((searchResult) => {
